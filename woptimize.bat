...
set "LOGFILE=%LOGDIR%\%APP%_%RANDOM%_%RANDOM%.log"
...
if "%choice%"=="4" goto services_menu
if "%choice%"=="5" goto network_menu
if "%choice%"=="6" goto ram_menu
if "%choice%"=="0" exit 
...
echo.
echo   [1] Apply BALANCED preset
echo   [2] Apply EXTREME preset (more tweaks, more risk)
echo   [3] Create Restore Point (recommended)
echo   [0] Back
echo.
set /p choice=      SELECT OPTION:
if "%choice%"=="1" goto intel_cpu_apply
if "%choice%"=="2" goto intel_cpu_apply
if "%choice%"=="3" goto create_restore_point
...
echo.
set /p choice=      SELECT OPTION:
if "%choice%"=="1" goto amd_cpu_apply
if "%choice%"=="2" goto amd_cpu_apply
if "%choice%"=="3" goto create_restore_point
if "%choice%"=="0" goto cpu_menu
goto amd_cpu_menu
...
echo                                     ║   [1] BALANCED OPTIMIZATION (Recommended)        ║ 

echo                                     ║   [2] EXTREME OPTIMIZATION (EXTREME)      ║ 
...
:intel_cpu_apply
call :RequireAdmin || goto main_menu
cls 
...
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Kernel" /v "DpcWatchdogThreshold" /t REG_DWORD /d 2147483647 /f
...
reg add "HKLM\SYSTEM\CurrentControlSet\Control\PriorityControl" /v "IoPrioritySeparation" /t REG_DWORD /d 38 /f
...
echo                                     ║           NVIDIA BALANCED OPTIMIZATION           ║ 
...
echo Completed. Restart your PC to apply the changes. 
...
echo (If System Protection is disabled, this may fail.)
powershell -NoProfile -ExecutionPolicy Bypass -Command "Checkpoint-Computer -Description 'Woptimize_RestorePoint' -RestorePointType 'MODIFY_SETTINGS'" >> "%LOGFILE%" 2>&1
if errorlevel 1 (
  echo Failed to create restore point. See log:
  echo   %LOGFILE%
) else (
...
echo.
echo ==========================================================
echo   Admin rights required for this action.
echo   Close this window and re-run as Administrator,
echo   or choose the option below to re-launch elevated.
echo ==========================================================
echo.
choice /C RL /N /M "Press R to re-launch as admin, or L to go back: "
if errorlevel 2 exit /b 1
powershell -NoProfile -ExecutionPolicy Bypass -Command "Start-Process -FilePath '%~f0' -Verb RunAs"
exit /b 1

:: ======================
:: NETWORK MENU (rewritten)
:: ======================
:network_menu
cls
echo.
...
echo.
echo   [1] Apply SAFE network tweaks
echo   [2] Reset network stack (winsock/ip reset)
echo   [3] Flush DNS cache
echo   [0] Back
echo.
set /p choice=      SELECT OPTION:
if "%choice%"=="1" goto network_safe
if "%choice%"=="2" goto network_reset
if "%choice%"=="3" goto network_flushdns
if "%choice%"=="0" goto main_menu
goto network_menu

:network_safe
call :RequireAdmin || goto network_menu
cls
echo Applying SAFE network tweaks...
echo.
echo - Disable TCP heuristics
echo - Enable RSS
echo - Set AutoTuning to normal
echo - Disable timestamps
echo - Flush DNS
echo.
...
:: ======================
:: SERVICES MENU (rewritten wrapper)
:: ======================
:services_menu
cls
echo.
...
echo.
echo   [1] Open advanced Services/Tweaks menu
echo   [2] Create Restore Point (recommended)
echo   [0] Back
echo.
set /p choice=      SELECT OPTION:
...
echo.
echo   [1] Apply SAFE cleanup + memory tweaks
echo   [2] Temp cleanup only
echo   [3] Create Restore Point (recommended)
echo   [0] Back
echo.
set /p choice=      SELECT OPTION:
if "%choice%"=="1" goto ram_apply
if "%choice%"=="2" goto ram_temp_only
if "%choice%"=="3" goto create_restore_point
if "%choice%"=="0" goto main_menu
goto ram_menu

:ram_temp_only
call :RequireAdmin || goto ram_menu
cls
...
echo Applying RAM/Memory optimizations...
...
echo AMD GPU optimization in progress... 
...
rem (removed redundant @echo off)
...
echo                                     ║       NVIDIA EXTREME OPTIMIZATION         ║ 
...
echo Optimization completed. Please restart your system.
...
echo Note: If you notice bugs or FPS drops with the Extreme preset, re-run the Balanced preset to revert risky changes while keeping the safer tweaks. A restart is required for changes to take effect.
...
:: Network and power-saving tuning via registry 

:: Apply network settings per interface 
...
:: Advanced power-management and network adapter tuning 
...
for /f %%a in ('reg.exe query HKLM /v "*WakeOnMagicPacket" /s ^| findstr "HKEY"') do ( 
    ...
    :: Some specific values require a different type/value 
...
echo [B] Enable ALL Windows Services 

echo. 

echo [C] Disable NON-essential services and tasks 

echo [D] Enable NON-essential services and tasks 

echo. 

echo [E] Disable Windows Update 

echo [F] Enable Windows Update 

echo. 

echo [G] Disable remote access services 

echo [H] Enable remote access services 

echo. 

echo [I] Disable printing services 

echo [J] Enable printing services 

echo. 

echo [K] Disable BLUETOOTH 

echo [L] Enable BLUETOOTH 
...
echo [O] Enable Microsoft Store and Xbox 
...
if /I "%example%"=="R" goto R
if /I "%example%"=="X" goto X
